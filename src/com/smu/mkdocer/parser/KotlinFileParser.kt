...
class KotlinFileParser : FileParser {
    override fun parseFile(file: File): Collection<Doc>? {
        val docFinder = DocFinder(Language.KOTLIN)
    ...
    private fun generateDoc(docDeclaration: String, docString: String): Doc {
        ...
        val type = docDeclaration.getDocType(Language.KOTLIN) ?: Type.PROPERTY
        ...
        return KotlinDoc(declaration = docDeclaration,
                type = type,
                params = paramsData,
                author = authorData,
                version = version,
